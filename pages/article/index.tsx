...
const ArticleCard = dynamic(
    () => import('../../components/ArticleCard').then((mod) => mod.ArticleCard),
    { loading: loadingView }
)
    ...
    return (
        <ReCaptchaScope reCaptchaKey={props.recaptchaKey}>
            <Scaffold
                fabContent={
                    user &&
                    hasPermission(user, 'post_article') && (
                        <>
                            <EditIcon sx={{ mr: 1 }} />
                            草拟
                        </>
...
function Content(props: { articles: RenderingArticle[] }): JSX.Element {
    ...
    if (props.articles.length > 0) {
        return (
            <Stack>
                ...
                <Grid
                    ...
                    animate={
                        !loadingProceeding && proceeding ? 'shown' : 'hidden'
                    }
                    variants={{
                        shown: { y: 0, opacity: 1 },
        ...
        return <PlaceHolder icon={NoArticleIcon} title="未提供文章" />
    }
}

export const getStaticProps: GetStaticProps<PageProps> = async () => {
    const articles = await listArticles().then((list) =>
        list
            .filter(ArticleUtil.publicList())
            .sort(
                (a, b) =>
                    Math.log(b.likes.length + 1) -
                    Math.log(b.dislikes.length + 1) -
                    Math.log(a.likes.length + 1) +
                    Math.log(a.dislikes.length + 1)
            )
    )
    const users = await getUsers(articles.map((m) => m.author))
    const unfolded: RenderingArticle[] = articles.map((m) =>
        users(m.author)
            ? {
                  ...getSafeArticle(m),
...
async function getRenderingArticle(
    meta: SafeArticle[]
): Promise<RenderingArticle[]> {
    let nicknameOf: {
        [key: string]: string
    } = {}
