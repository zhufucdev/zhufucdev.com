...
export function ArticleHeader(props: {
    ...
    useEffect(() => {
        if (!props.article.current || !props.article.current.children[0]) {
            setContents(undefined)
            return
        }

        const gen = generateNodeTree(props.article.current.children[0])
        setContents(gen)
    ...
    return (
        <>
            ...
            {onWideScreen && contents && (
                <motion.div
                    animate={{
                        y: scrolled || !Boolean(props.meta.cover) ? 0 : 180,
                    }}
                    style={{
                        width: 240,
                        position: 'fixed',
                ...
                >
                    <List
                        sx={{ '.MuiListItemButton-root': { borderRadius: 4 } }}
                        dense
                    >
