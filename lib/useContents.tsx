...
const ContentsInstance = React.createContext<ContentsContext>([undefined, () => {
}]);
...
export type ContentsContext = [ContentsRootNode | undefined, (contents?: Contents) => void];
...
export function ContentsProvider(props: { children: React.ReactNode }) {
    ...
    const contents = useMemo<ContentsContext>(() =>
        [root, (contents) => {
            if (contents && contents.nodes.length > 0) setRoot(new ContentsRootNode(contents))
